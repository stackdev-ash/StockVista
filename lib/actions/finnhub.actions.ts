  ...
  async (query?: string): Promise<StockWithWatchlistStatus[]> => {
    try {
      ...
      const mapped: StockWithWatchlistStatus[] = results
        .filter((r) => !r.symbol.endsWith(".NS"))
        .map((r) => {
          const upper = (r.symbol || "").toUpperCase();
          const name = r.description || upper;
          const exchange = (r as any).__exchange || "US";
      ...
      const uniqueStocks = Array.from(
        new Map(mapped.map((stock) => [stock.symbol, stock])).values(),
      );

      return uniqueStocks.slice(0, 15);
    } catch (err) {
...
export const getWatchlistStockData = cache(
  async (symbol: string) => {
    const quote = await fetchJSON(
      `${FINNHUB_BASE_URL}/quote?symbol=${symbol}&token=${NEXT_PUBLIC_FINNHUB_API_KEY}`,
      60,
    );
